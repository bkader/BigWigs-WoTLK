...
local options = BigWigs:NewModule("Options", "AceEvent-3.0")
options:SetEnabledState(true)
_G.BigWigsOptions = options
...
function options:OnInitialize()
	...
	about:SetScript("OnShow", function(frame)
		...
		for i, field in next, fields do
			local t = frame:CreateFontString(nil, "ARTWORK", "GameFontNormal")
			t:SetWidth(120)
			if not anchor then
				t:SetPoint("TOPLEFT", subtitle, "BOTTOMLEFT", 0, -16)
			else
				t:SetPoint("TOP", anchor, "BOTTOM", 0, -6)
				t:SetPoint("LEFT", subtitle)
			end
			t:SetNonSpaceWrap(true)
			t:SetJustifyH("LEFT")
			t:SetText(field)
			local detail = frame:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
			detail:SetPoint("TOPLEFT", t, "TOPRIGHT")
