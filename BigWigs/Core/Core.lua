...
local function targetCheck(unit)
	if not UnitName(unit) or UnitIsCorpse(unit) or UnitIsDead(unit) or UnitPlayerControlled(unit) then
		return
	end
...
do
	...
	function addon:GetEnableMobs()
		return enablemobs
	end
	function addon:GetEnableYells()
		return enableyells
	end
end
...
function addon:Translate(boss)
	if LOCALE ~= "enUS" and BB and BB[boss] then
		return BB[boss]
	end
	return boss
end
...
function addon:NewBossLocale(name, locale, default)
	return AL:NewLocale(string.format("%s_%s", self.bossCore.name, name), locale, default)
end
...
do
	...
	function addon:IterateBossModules()
		return self.bossCore:IterateModules()
	end
	function addon:GetBossModule(...)
		return self.bossCore:GetModule(...)
	end

	function addon:IteratePlugins()
		return self.pluginCore:IterateModules()
	end
	function addon:GetPlugin(...)
		return self.pluginCore:GetModule(...)
	end
		...
		if module.toggleOptions then
			...
			for k, v in next, module.toggleOptions do
				...
				elseif t == "number" and v > 1 then
					...
					if not n then
						error(("Invalid spell ID %d in the toggleOptions for module %s."):format(v, module.name))
					end
	...
	function addon:RegisterBossModule(module)
		if not module.displayName then
