...
function loader:OnInitialize()
	for i = 1, GetNumAddOns() do
		...
		if enabled and not IsAddOnLoaded(i) and IsAddOnLoadOnDemand(i) then
			...
			meta = GetAddOnMetadata(i, "X-BigWigs-LoadOn-ZoneID") or GetAddOnMetadata(i, "X-BigWigs-LoadOn-Zone") or GetAddOnMetadata(i, "X-BigWigs-LoadOn-ZoneType")
			if meta then
...
function loader:OnEnable()
	...
	if loadOnZoneAddons then
		...
		for i, name in next, loadOnZoneAddons do
			...
			if menu then
				if BZ then
					menu = BZ[menu] or menu
				end
				if not loadOnZone[menu] then
					loadOnZone[menu] = {}
				end
				menus[menu] = true
			end
			...
			local meta =
				GetAddOnMetadata(name, "X-BigWigs-LoadOn-Zone") or GetAddOnMetadata(name, "X-BigWigs-LoadOn-ZoneType")
...
do
	...
	function loader:CHAT_MSG_ADDON(event, prefix, message, distribution, sender)
		...
		elseif prefix == "BWOOD3" then
			if not tonumber(message) or warnedOutOfDate then
				return
			end
		...
		elseif prefix == "BWVR3" then
			...
			if message > highestReleaseRevision then
				highestReleaseRevision = message
			end
...
function loader:BigWigs_CoreEnabled()
	-- coreEnabled = true
	if ldb then
		ldb.icon = "Interface\\AddOns\\BigWigs\\Media\\Textures\\icons\\core-enabled"
	end
...
function loader:BigWigs_CoreDisabled()
	-- coreEnabled = false
	if ldb then
		ldb.icon = "Interface\\AddOns\\BigWigs\\Media\\Textures\\icons\\core-disabled"
	end
end
