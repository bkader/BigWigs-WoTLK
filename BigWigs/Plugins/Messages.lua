...
plugin.defaultDB = {
	...
	useicons = true,
	classcolor = true,
	emphasizedMessages = {sink20OutputSink = "BigWigsEmphasized"}
}
...
function plugin:OnPluginEnable()
	...
	self:RegisterMessage("BigWigs_StopConfigureMode", function() anchor:Hide() end)
...
function plugin:Print(addon, text, r, g, b, font, size, _, _, _, icon)
	...
	local fface, fsize, fflags = f:GetFont()
	slot:SetFont(fface, fsize, fflags or "THICKOUTLINE")

	--slot:SetFontObject(font or GameFontNormalHuge)
	slot.minHeight = select(2, slot:GetFont())
	if icon then
		text = iconString:format(icon, slot.minHeight, slot.minHeight, -5) .. text
	end
	slot:SetText(text)
...
function plugin:BigWigs_Message(event, module, key, text, color, _, sound, broadcastonly, icon)
	if broadcastonly or not text then
		return
	end
