...
local function disable()
	return not plugin.db.profile.show
end
...
do
	...
	local function getURL()
		return select(3, footer:GetText():find("cff9999ff(.*)|r"))
	end
...
function plugin:CHAT_MSG_ADDON(event, prefix, message, distribution, sender)
	if prefix ~= "BWTIP" or not self.db.profile.manual then
		return
	end
	if not UnitIsRaidOfficer(sender) then
		return
	end
...
do
	...
	local DEVELOPER_COLOR = {
		-- XXX Find a better color, this one looks crap!
		r = 0.4,
		g = 0.7,
		b = 0.4
	}
