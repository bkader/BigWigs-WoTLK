...
local function rearrangeBars(anchor)
	...
	local up, spacing = nil, 0
	if anchor == normalAnchor then
		up = db.growup
		spacing = db.spacing
	else
		up = db.emphasizeGrowup
		spacing = db.emphasizeSpacing
	end
	for i, bar in next, tmp do
		...
		if up or (db.emphasizeGrowup and bar:Get("bigwigs:emphasized")) then
			...
			bar:SetPoint("BOTTOMRIGHT", lastUpBar or anchor, "TOPRIGHT", spacing)
			lastUpBar = bar
		else
...
do
	...
	function plugin:GetPluginConfig()
		...
		do
			...
			height:SetValue(db.height or 14)
			height:SetSliderValues(8, 50, 1)
			height:SetLabel(L["Height"])
			height:SetUserData("key", "height")
			height:SetCallback("OnValueChanged", standardCallback)
			height:SetFullWidth(true)
			normal:AddChildren(height)

			local spacing = AceGUI:Create("Slider")
			spacing:SetValue(db.spacing or 0)
			spacing:SetSliderValues(0, 20, 1)
			spacing:SetLabel(L["Spacing"])
			spacing:SetUserData("key", "spacing")
			spacing:SetCallback("OnValueChanged", standardCallback)
			spacing:SetFullWidth(true)
			normal:AddChildren(spacing)
		end
		...
		do
			...
			local height = AceGUI:Create("Slider")
			height:SetValue(db.emphasizeHeight or 20)
			height:SetSliderValues(8, 50, 1)
			height:SetLabel(L["Height"])
			height:SetUserData("key", "emphasizeHeight")
			height:SetCallback("OnValueChanged", standardCallback)
			height:SetFullWidth(true)

			local spacing = AceGUI:Create("Slider")
			spacing:SetValue(db.emphasizeSpacing or 0)
			spacing:SetSliderValues(0, 20, 1)
			spacing:SetLabel(L["Spacing"])
			spacing:SetUserData("key", "emphasizeSpacing")
			spacing:SetCallback("OnValueChanged", standardCallback)
			spacing:SetFullWidth(true)

			emphasize:AddChildren(enable, flash, move, growup, scale, height, spacing)
		end
...
local function countdown(bar)
	if bar.remaining <= bar:Get("bigwigs:count") then
		...
		PlaySoundFile("Interface\\AddOns\\BigWigs\\Media\\Sounds\\"..floor(count)..".mp3")
		if count > 0.9 then
...
function plugin:BigWigs_StartBar(message, module, key, text, time, icon)
	if not normalAnchor then createAnchors() end
	stop(module, text)
	local bar = candy:New(media:Fetch("statusbar", db.texture), 200, db.height or 14)
...
do
	...
	function plugin:EmphasizeBar(bar)
		...
		if db.emphasizeFlash then
			bar:AddUpdateFunction(eflash)
		end
		bar:SetColor(unpack(bar:Get("bigwigs:empColor")))
		bar:SetScale(db.emphasizeScale)
		bar:SetHeight(db.emphasizeHeight)
		bar.candyBarIconFrame:SetWidth(db.emphasizeHeight)
...
local function parseTime(input)
	...
	if type(input) == "string" then
		...
		elseif input:find("^%d+se?c?$") then
			return tonumber(select(3, input:find("^(%d+)se?c?$")))
		end
	end
end
