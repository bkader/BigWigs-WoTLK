...
local UnitRace, UnitClass = UnitRace, UnitClass
local UnitIsDeadOrGhost = UnitIsDeadOrGhost
local GetTime = GetTime
local setmetatable = setmetatable
local type, pairs, next = type, pairs, next
local GetSpellInfo, IsSpellInRange = GetSpellInfo, IsSpellInRange
...
local function getClosestRangeFunction(toRange)
	if ranges[toRange] then
		return ranges[toRange], toRange
	end
	local closest = 15
	local closestDiff = math_abs(toRange - 15)
	for range, func in pairs(ranges) do
		local diff = math_abs(toRange - range)
...
local function setConfigureTarget(self, button)
	if not inConfigMode or button ~= "LeftButton" then
		return
	end
...
local function onNormalClose()
	if activeProximityFunction then
...
do
	...
	local function updateProximity()
		...
		for i = 1, num do
			...
			if
				n and UnitExists(n) and not UnitIsDeadOrGhost(n) and not UnitIsUnit(n, "player") and
					activeProximityFunction(n)
			 then
				tooClose[#tooClose + 1] = coloredNames[n]
			end
			if #tooClose > 4 or i > 25 then
				break
			end
		end
		...
		if #tooClose == 0 then
			...
			wipe(tooClose)
			if not plugin.db.profile.sound then
				return
			end
			local t = GetTime()
...
do
	...
	function plugin:BigWigs_ShowProximity(event, module, range)
		if self.db.profile.disabled or type(range) ~= "number" then
			return
		end
		opener = module
		self:Open(range)
...
function plugin:Open(range)
	if type(range) ~= "number" then
		error("Range needs to be a number!")
	end
