...
do
	...
	local function blazingSpawn()
		...
		if blazingRepeater or (not IsRaidLeader() and not IsRaidOfficer()) then
			return
		end
...
do
	...
	function mod:ManaVoid(player, spellId)
		if UnitIsUnit(player, "player") and (self:IsHealer() or self:IsRanged()) and (GetTime() - t > 2) then
			t = GetTime()
