...
mod.toggleOptions = {
	72143,
	70541,
	{73912, "SAY", "ICON", "FLASHSHAKE"},
	70372,
	{72762, "SAY", "ICON", "WHISPER", "FLASHSHAKE"},
	{69409, "SAY"},
	69037,
	{68980, "ICON", "WHISPER", "FLASHSHAKE"},
	70498,
	{74270, "FLASHSHAKE"},
	{69200, "SAY"},
	{72262, "FLASHSHAKE"},
	72350,
	{73529, "SAY", "WHISPER", "FLASHSHAKE", "ICON"},
	"berserk",
	"bosskill"
}
...
function mod:PlagueTick(horrorName, _, _, tickDamage, _, _, _, _, _, dGUID)
	if self:GetInstanceDifficulty() < 3 then
		return
	end -- Doesn't apply on normal diff.
	-- Not ticking on a Shambling Horror, so bail early
	if tonumber(dGUID:sub(-12, -7), 16) ~= 37698 then
		return
	end

	if not plagueTicks[dGUID] then
		plagueTicks[dGUID] = 1
	else
		plagueTicks[dGUID] = plagueTicks[dGUID] + 1
	end
...
function mod:Infest(_, spellId, _, _, spellName)
	...
	self:Bar(70541, L["infest_bar"], 22.5, spellId)
end
...
function mod:NecroticPlague(player, spellId, _, _, spellName)
	...
	if UnitIsUnit(player, "player") then
		self:Voice(73912, true)
		self:FlashShake(73912)
	end
	self:Bar(73912, L["necroticplague_bar"], 31.5, spellId)
		...
		for i = 1, GetNumRaidMembers() do
			...
			if player then
				...
				if debuffed and (expire - GetTime()) > 13 then
					...
					if UnitIsUnit(player, "player") then
						mod:Voice(73912, true)
						mod:FlashShake(73912)
					end
...
function mod:RagingSpirit(player, spellId, _, _, spellName)
	...
	self:Bar(69200, L["ragingspirit_bar"], 22.5, spellId)
end
...
function mod:HarvestSoul(player, spellId, _, _, spellName)
	if self:GetInstanceDifficulty() > 2 then
		...
		if UnitIsUnit(player, "player") then
			self:Voice(68980, true)
			self:FlashShake(68980)
		else
			self:Voice(68980)
		end
...
do
	...
	function mod:DefileCast(player, spellId, _, _, spellName)
		...
		self:Bar(72762, L["defile_bar"], 33.5, 72762)
...
do
	...
	function mod:ShadowTrap(_, spellId, _, _, spellName)
		if not scheduled then
			...
			self:Bar(73529, L["trap_bar"], 15.5, spellId)
		end
	end
end